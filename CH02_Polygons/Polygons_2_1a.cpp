...
{
	Draw3DPolygon( -1.0f, -1.0f, 0.0f, 0xffff0000, 1.0f, -1.0f, 0.0f, 0xff00ff00, 0.0f, 1.0f, 0.0f, 0xff0000ff );
...
int Draw3DPolygon( float x1, float y1, float z1, int nColor1, float x2, float y2, float z2, int nColor2, float x3, float y3, float z3, int nColor3 )
{
	if( g_nVertexNum > ( MAX_BUFFER_VERTEX - 3 ) )
		return -1; // 정점이 버퍼로부터 넘칠 경우는 그리지 않는다
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_2_1" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
