...
int RenderScanLine( void )
{
	...
	{ // y방향루프
		...
		{																																																												// x방향루프
			GetBMPPixel( (int)( cx + VIEW_WIDTH / 2.0f ), (int)( cy + VIEW_HEIGHT / 2.0f ), &mbPicture, &nRed, &nGreen, &nBlue ); // 점 획득
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Render_5_1" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
