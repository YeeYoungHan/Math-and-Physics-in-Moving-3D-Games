...
int GetPixelFiltered( float x, float y, MemoryBMP *pmbSrc, int *pnRed, int *pnGreen, int *pnBlue )
{
	...
	GetBMPPixel( (int)x, (int)y, pmbSrc, &nRed1, &nGreen1, &nBlue1 );					// 점1획득
	GetBMPPixel( (int)x + 1, (int)y, pmbSrc, &nRed2, &nGreen2, &nBlue2 );			// 점2획득
	GetBMPPixel( (int)x, (int)y + 1, pmbSrc, &nRed3, &nGreen3, &nBlue3 );			// 점3획득
	GetBMPPixel( (int)x + 1, (int)y + 1, pmbSrc, &nRed4, &nGreen4, &nBlue4 ); // 점4획득
...
int RenderScanLine( void )
{
	...
	for( y = 0; y < VIEW_HEIGHT; y++ )
	{
		...
		{
			GetBMPPixel( (int)( cx + VIEW_WIDTH / 2.0f ), (int)( cy + VIEW_HEIGHT / 4.0f ), &mbPicture, &nRed, &nGreen, &nBlue ); // 점 획득
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Render_6_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
