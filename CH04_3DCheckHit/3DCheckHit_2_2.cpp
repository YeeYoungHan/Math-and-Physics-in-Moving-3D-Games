...
// 충돌판정 삼각형 정점
XMFLOAT3 g_TriangleVertices[CHECK_TRIANGLE_NUM * 3] = { XMFLOAT3( 2.0f, 0.01f, -2.0f ), XMFLOAT3( -2.0f, 0.01f, -2.0f ), XMFLOAT3( 0.0f, 0.01f, 1.5f ),
																												XMFLOAT3( 6.0f, 0.01f, -2.5f ), XMFLOAT3( 2.5f, 0.01f, -1.0f ),	 XMFLOAT3( 4.0f, 0.01f, 4.5f ),
																												XMFLOAT3( -1.0f, 0.01f, 5.0f ), XMFLOAT3( -5.0f, 0.01f, -0.5f ), XMFLOAT3( -1.0f, 0.01f, 1.0f ) };
...
int CheckHit( XMFLOAT3 *pv3Triangle, XMFLOAT3 *pv3Point )
{
	...
	{
		// 플레이어 위치 제어
		if( ( fabsf( fCross0 ) <= fabsf( fCross1 ) ) && ( fabsf( fCross0 ) <= fabsf( fCross2 ) ) )
		{
			// 변０에 가장 가깝다
...
HRESULT InitGeometry( void )
{
	...
	{
		g_cvVertices[g_nVertexNum + nIndex].v4Pos = XMFLOAT4( g_TriangleVertices[i].x, g_TriangleVertices[i].y, g_TriangleVertices[i].z, 1.0f );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_2_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
