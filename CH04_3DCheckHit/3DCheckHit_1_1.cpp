...
int CheckHit( Sphere *pCheckSphere1, Sphere *pCheckSphere2 )
{
	...
	if( fDistanceSq < ( pCheckSphere1->r + pCheckSphere2->r ) * ( pCheckSphere1->r + pCheckSphere2->r ) )
	{
		nResult = true;
	}
	else
...
int MakeSphereIndexed( CUSTOMVERTEX *pVertices, int *pVertexNum, WORD *pIndices, int *pIndexNum ) // 구 만들기(인덱스된)
{
	int i, j;
	float fTheta;
	float fPhi;
	...
	for( i = 0; i < CORNER_NUM / 2 + 1; i++ )
	{
		...
		{
			pVertices[nIndex].v4Pos = XMFLOAT4( sinf( fTheta ) * cosf( fPhi ), cosf( fTheta ), sinf( fTheta ) * sinf( fPhi ), 1.0f );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_1_1" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
