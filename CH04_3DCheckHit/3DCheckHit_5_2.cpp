...
XMMATRIX CreateWorldMatrix( float x, float y, float z, float fSize ) // 월드 행렬 생성
{
	...
	XMMATRIX matRot_Y;					 // y軸周り回?行列
	XMMATRIX matRot_X;					 // x軸周り回?行列
	XMMATRIX matScaleTrans;			 // ?大縮小平行移動行列
...
D3D_FEATURE_LEVEL g_FeatureLevel;												 // 피처레벨フィ?チャ?レベル
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_5_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
