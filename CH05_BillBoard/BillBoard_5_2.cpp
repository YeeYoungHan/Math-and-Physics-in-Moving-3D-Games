...
int MakeGroundNormals( CUSTOMVERTEX *pVertices, WORD *pIndices, int nIndexNum )
{
	...
	for( i = 0; i < nIndexNum; i += 3 )
	{
		...
		nIndex_X = nIndex1 % HEIGHT_NUM;
		nIndex_Z = nIndex1 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
		nIndex_X = nIndex2 % HEIGHT_NUM;
		nIndex_Z = nIndex2 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
		nIndex_X = nIndex3 % HEIGHT_NUM;
		nIndex_Z = nIndex3 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
	}
...
int Make3DRectangleIndexed( float fWidth, float fHeight, float fDepth, int bCenterMode, CUSTOMVERTEX *pVertices, int *pVertexNum, WORD *pIndices, int *pIndexNum,
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "BillBoard_5_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
