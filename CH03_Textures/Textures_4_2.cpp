...
int DrawChangingPictures( void )
{
	...
	for( i = 0; i < CORNER_NUM + 1; i++ )
	{
		...
		for( j = 0; j < CORNER_NUM + 1; j++ )
		{
			...
			Vertices[nIndex].v2UV = XMFLOAT2( fTheta / ( 2.0f * PI ), vs + (float)i / CORNER_NUM );
			nIndex++;
			fTheta += fAngleDelta;
		}
	}
	...
	// 정점 인덱스로 그리기
	DrawIndexed3DPolygonsTex( Vertices, ( CORNER_NUM + 1 ) * ( CORNER_NUM + 1 ), wIndices, CORNER_NUM * CORNER_NUM * 2 * 3 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Textures_4_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
