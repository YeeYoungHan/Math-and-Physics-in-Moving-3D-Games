...
int MakeSphereIndexed( CUSTOMVERTEX *pVertices, int *pVertexNum, WORD *pIndices, int *pIndexNum ) // 구 작성(인덱스)
{
	int i, j;
	float fTheta;
	float fPhi;
	...
	for( i = 0; i < CORNER_NUM / 4 + 1; i++ )
	{
		...
		{
			pVertices[nIndex].v4Pos = XMFLOAT4( R * sinf( fTheta ) * cosf( fPhi ), R * cosf( fTheta ), R * sinf( fTheta ) * sinf( fPhi ), 1.0f );
	...
	for( i = 0; i < MAX_EXPLOSION_NUM; i++ )
	{
		if( exExplode[i].bActive )
		{
			...
			else
			{
				...
				{
					exExplode[i].fBright = ( EXPLOSION_R2 - exExplode[i].fRadius ) / ( EXPLOSION_R2 - EXPLOSION_R1 );
				}
...
HRESULT InitGeometry( void )
{
	...
	MakeSphereIndexed( (CUSTOMVERTEX *)mappedVertices.pData, &g_nVertexNum, (WORD *)mappedIndices.pData, &g_nIndexNum );
	g_pImmediateContext->Unmap( g_pVertexBuffer, 0 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Textures_5_3" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
