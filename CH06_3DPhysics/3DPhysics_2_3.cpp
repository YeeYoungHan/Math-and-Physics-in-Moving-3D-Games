...
int MovePlayer( void ) // 구의 이동
{
	...
	if( !( Shell_1.bActive ) )
	{
		...
		Shell_1.v3Vel = XMFLOAT3( SHELL_FIRST_VEL * cosf( Player_1.fTheta ) * cosf( -Player_1.fPhi ), SHELL_FIRST_VEL * sinf( Player_1.fTheta ),
...
HRESULT Render( void )
{
	...
	g_mmGunbarrel.mMatrix = XMMatrixRotationZ( -PI / 2.0f + Player_1.fTheta ) * XMMatrixRotationY( Player_1.fPhi );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_2_3" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
