...
int MovePlayer( void ) // 추의 이동
{
	...
	for( i = 0; i < DIVIDE_T; i++ )
	{
		...
		g_Weights[0].v3Vel.y += ( -SPRING_K * ( g_Weights[0].v3Pos.y - CENTER_HEIGHT ) - fSpring2F ) / DIVIDE_T;
		g_Weights[1].v3Pos.y += g_Weights[1].v3Vel.y / DIVIDE_T;
		g_Weights[1].v3Vel.y += ( -SPRING_K * ( g_Weights[1].v3Pos.y - CENTER_HEIGHT ) + fSpring2F ) / DIVIDE_T;
	}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_4_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
