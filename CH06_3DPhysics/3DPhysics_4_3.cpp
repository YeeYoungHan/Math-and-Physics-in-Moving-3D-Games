...
int MovePlayer( void )
{
	...
	for( i = 0; i < DIVIDE_T; i++ )
	{
		...
		for( j = 1; j < WEIGHT_NUM - 1; j++ )
		{
			...
			g_Weights[j].v3Vel.y += ( -SPRING_K * ( g_Weights[j].v3Pos.y - CENTER_HEIGHT ) - fSpring_SF[j - 1] + fSpring_SF[j] ) / DIVIDE_T;
		}
	}
...
HRESULT InitGeometry( void )
{
	...
	Make3DRectangleIndexed( 1.0f, 0.7f, 1.0f, true, &( g_cvVertices[g_nVertexNum] ), &nVertexNum1, &( g_wIndices[g_nIndexNum] ), &nIndexNum1, 0 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_4_3" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
