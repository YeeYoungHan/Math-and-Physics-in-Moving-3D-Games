	...
	for( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ )
	{
		...
		for( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ )
		{
			...
			for( k = 0; k < WAVE_NUM; k++ )
			{
				...
				g_fHeights[i][j] += Waves[k].fAmp * sinf( 2.0f * PI * ( fTime * fFreq - ( x * Waves[k].kx + z * Waves[k].kz ) ) + Waves[k].fDelta );
			}
			x += BLOCK_WIDTH;
		}
		z += BLOCK_WIDTH;
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_6_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
