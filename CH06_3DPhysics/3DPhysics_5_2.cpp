...
int MakeHeights( void )
{
	...
	for( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ )
	{
		...
		{
			g_fHeights[i][j] = 3.0f * sinf( 2.0f * PI * ( fTime / ROUND_TIME - x / GROUND_SIZE ) );
			x += BLOCK_WIDTH;
		}
		z += BLOCK_WIDTH;
	}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
	...
	g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_5_2" ), WS_OVERLAPPEDWINDOW, 100, 20, rcRect.right - rcRect.left, rcRect.bottom - rcRect.top, GetDesktopWindow(), NULL,
												 wc.hInstance, NULL );
